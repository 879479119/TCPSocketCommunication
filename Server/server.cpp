...
DWORD WINAPI TcpServeThread(LPVOID lpParam)
{
	...
	cout << "New Client Linked" <<endl << endl;
	...
	while (TRUE)
	{
		...
		cout << ServerTCPBuf << endl;

		
		if (strcmp("xxxxx----", strncpy(header, ServerTCPBuf, 9)) == 0)
		{
			//获取系统时间并发送给client
			memset(ServerTCPBuf, '\0', sizeof(ServerTCPBuf));
			switch (count) {
			case 0:
				strcpy(ServerTCPBuf, "你好");
				break;
			case 1:
				strcpy(ServerTCPBuf, "谢谢");
				break;
			case 2:
				strcpy(ServerTCPBuf, "拜拜");
				break;
			case 3:
				strcpy(ServerTCPBuf, "科科");
				break;
			default:
				strcpy(ServerTCPBuf, "没了");
				break;
			}
			send(TcpSocket, ServerTCPBuf, strlen(ServerTCPBuf), 0);
			count++;
