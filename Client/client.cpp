...
int main(int argc, char* argv[])
{
	...
	if ((RetValue=WSAConnect(cTCPSocket,(sockaddr *)&TCPServer,sizeof(TCPServer),NULL,NULL,NULL,(LPQOS)NULL) )==SOCKET_ERROR)
	{
	...
	while(TRUE)
	{
		...
		case '4':  //向全局广播
			...
			cout << "回车后发送消息与服务器聊天" << endl << "如果想要退出本模式请输入 out" << endl;
			...
			while (TRUE) {
				...
				if (ClientBuf[0] == 'o' &&ClientBuf[1] == 'u' &&ClientBuf[2] == 't') {
					cout << "out Success" << endl;
					break;
				}
				
				char header[sizeof(ClientBuf)+10] = "xxxxx----";
				strcat(header, ClientBuf);

				if ((BytesSent = send(cTCPSocket, header, strlen(header), 0)) == SOCKET_ERROR)
				{
					printf("send() failed for cTCPSocket with error %d\n", WSAGetLastError());
					printf("Can not send command to server by TCP.Maybe Server is down.\n");
					return 7;
